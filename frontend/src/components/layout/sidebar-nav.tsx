...
export function SidebarNav({ onNavigate, collapsed = false }: SidebarNavProps) {
  ...
  return (
    <nav className={cn("flex flex-col", collapsed ? "items-center gap-1 px-1.5" : "gap-3 px-3")}>
      {navSections.map((section, sIdx) => {
        const sectionTitle = t(section.titleKey)

        return (
          <div key={section.id} className={cn("flex flex-col", collapsed ? "w-full items-center gap-1.5" : "gap-1")}>
            {/* Section Heading or Divider */}
            {collapsed ? (
              sIdx > 0 && <div className="my-1 w-6 h-px bg-border/70" />
            ) : (
            ...
            {/* Section Items */}
            <div className={cn("flex flex-col", collapsed ? "w-full items-center gap-1" : "gap-0.5")}>
